...
export async function fetchTransactions(institutionId: string, filters?: {
  ...
  return deduplicateRequest(key, async () => {
    ...
    // Use safeQuery to handle execution and notifications
    // We disable the default generic toast for fetch because we want the UI to handle empty states/loading
    // BUT we want safeQuery to handle RLS/Network errors with specific toasts.
    // Actually safeQuery defaults showToast=true.
    // For a list fetch, a toast on failure is good.
    const { data, error } = await safeQuery(query, {
      errorMessage: 'Failed to retrieve transactions'
    });

    if (error) {
      // Re-throw so React Query enters error state
      throw error;
    }
...
export async function allocateTransaction(params: AllocateTransactionParams) {
  ...
  return deduplicateRequest(key, async () => {
    // 1. Perform Allocation (RPC)
    const { error: rpcError } = await safeQuery(
      supabase.rpc('allocate_transaction', {
        p_transaction_id: params.transaction_id,
        p_member_id: params.member_id,
        p_note: params.note || null
      }),
      { errorMessage: 'Failed to allocate transaction' }
    );
    ...
    // 2. Fetch updated transaction details
