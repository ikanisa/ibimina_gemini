...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchProfile = async (nextUser: User): Promise<SupabaseProfile | null> => {
    ...
    return deduplicateRequest(key, async () => {
      ...
      // Wrap with timeout and safeQuery
      // We disable default toast here because AuthContext handles critical init errors specifically
      const { data, error } = await safeQuery(
        ...
        { showToast: false, errorMessage: 'Failed to load user profile' }
      );
  ...
  const applyUser = async (nextUser: User | null) => {
    ...
    try {
      ...
      if (nextProfile) {
        ...
        // Use safeQuery to handle any potential network errors gracefully even in background
        safeQuery(
          supabase
            .from('profiles')
