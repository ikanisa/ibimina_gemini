...
-- Ensure columns exist for optimized indexes
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'momo_sms_raw' AND column_name = 'processed_at') THEN
    ALTER TABLE public.momo_sms_raw ADD COLUMN processed_at timestamptz DEFAULT NULL;
  END IF;
END $$;
...
COMMENT ON INDEX idx_transactions_allocation_status_optimized IS 
...
COMMENT ON INDEX idx_transactions_unallocated_optimized IS 
...
COMMENT ON INDEX idx_transactions_member_history_optimized IS 
'Composite index for member transaction history queries. Optimizes queries showing all transactions for a specific member.';

COMMENT ON INDEX idx_momo_sms_raw_pending_processing_optimized IS 
