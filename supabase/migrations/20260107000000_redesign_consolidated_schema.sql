...
DO $$
BEGIN
  -- Migrate contributions → transactions (if table exists)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'contributions') THEN
    INSERT INTO public.transactions (
      id, institution_id, member_id, group_id, type, amount, currency, channel,
      status, reference, occurred_at, created_at, allocation_status
    )
    SELECT 
      id, institution_id, member_id, group_id, 'CONTRIBUTION', amount, 'RWF', 
      ...
      CASE status
        ...
        ELSE 'PENDING'
      END,
      reference, date::date, created_at,
      CASE WHEN member_id IS NOT NULL THEN 'allocated'::transaction_allocation_status ELSE 'unallocated'::transaction_allocation_status END
    FROM public.contributions
    WHERE NOT EXISTS (SELECT 1 FROM public.transactions t WHERE t.id = contributions.id)
    ON CONFLICT (id) DO NOTHING;
    RAISE NOTICE 'Migrated contributions to transactions';
  ELSE
    RAISE NOTICE 'Table contributions does not exist, skipping migration';
  END IF;

  -- Migrate incoming_payments → transactions (if table exists)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'incoming_payments') THEN
    INSERT INTO public.transactions (
      id, institution_id, type, amount, currency, channel, status,
      reference, occurred_at, created_at, payer_ref, allocation_status
    )
    SELECT 
      id, institution_id, 'PAYMENT', amount, 'RWF', 'MoMo',
      CASE status
        WHEN 'UNRECONCILED' THEN 'PENDING'
        WHEN 'RECONCILED' THEN 'COMPLETED'
        ELSE 'PENDING'
      END,
      reference, received_date::date, created_at, payer_ref, 'unallocated'::transaction_allocation_status
    FROM public.incoming_payments
    WHERE NOT EXISTS (SELECT 1 FROM public.transactions t WHERE t.id = incoming_payments.id)
    ON CONFLICT (id) DO NOTHING;
    RAISE NOTICE 'Migrated incoming_payments to transactions';
  ELSE
    RAISE NOTICE 'Table incoming_payments does not exist, skipping migration';
  END IF;

  -- Migrate payment_ledger → transactions (if table exists)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'payment_ledger') THEN
    INSERT INTO public.transactions (
      id, institution_id, member_id, group_id, type, amount, currency,
      channel, status, reference, occurred_at, created_at, payer_ref, allocation_status
    )
    SELECT 
      id, institution_id, member_id, group_id, 
      COALESCE(txn_type, 'PAYMENT'), amount, currency, 'MoMo',
      CASE WHEN reconciled THEN 'COMPLETED' ELSE 'PENDING' END,
      reference, timestamp, created_at, counterparty,
      CASE WHEN member_id IS NOT NULL THEN 'allocated'::transaction_allocation_status ELSE 'unallocated'::transaction_allocation_status END
    FROM public.payment_ledger
    WHERE NOT EXISTS (SELECT 1 FROM public.transactions t WHERE t.id = payment_ledger.id)
    ON CONFLICT (id) DO NOTHING;
    RAISE NOTICE 'Migrated payment_ledger to transactions';
  ELSE
    RAISE NOTICE 'Table payment_ledger does not exist, skipping migration';
  END IF;

  -- Migrate sms_messages → momo_sms_raw (if table exists)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'sms_messages') THEN
    INSERT INTO public.momo_sms_raw (
      id, institution_id, sender_phone, sms_text, received_at, source,
      hash, parse_status, parse_error, created_at
    )
    SELECT 
      id, institution_id, sender, body, timestamp, 'manual_import'::sms_source,
      encode(digest(concat(sender, body, timestamp::text), 'sha256'), 'hex'),
      CASE 
        WHEN is_parsed THEN 'success'::sms_parse_status
        ELSE 'pending'::sms_parse_status
      END,
      null, created_at
    FROM public.sms_messages
    WHERE NOT EXISTS (SELECT 1 FROM public.momo_sms_raw m WHERE m.id = sms_messages.id)
    ON CONFLICT (hash) DO NOTHING;

    -- Link transactions to SMS if they were linked
    UPDATE public.transactions t
    SET momo_sms_id = sm.linked_transaction_id
    FROM public.sms_messages sm
    WHERE t.id = sm.linked_transaction_id
      AND t.momo_sms_id IS NULL;
    RAISE NOTICE 'Migrated sms_messages to momo_sms_raw';
  ELSE
    RAISE NOTICE 'Table sms_messages does not exist, skipping migration';
  END IF;
END $$;
