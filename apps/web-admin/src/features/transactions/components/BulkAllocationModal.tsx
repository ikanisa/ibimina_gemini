...
import { X, User, Users, Check, Loader2, AlertCircle, Layers } from 'lucide-react';
...
export const BulkAllocationModal: React.FC<BulkAllocationModalProps> = ({
    ...
    const { bulkAllocate, isBulkAllocating } = useAllocateTransaction();

    const handleBulkAllocate = async () => {
        if (!selectedMember || selectedIds.size === 0) return;
        setError(null);
        ...
        try {
            ...
            setError(err.message || 'Failed to bulk allocate transactions');
        }
    };
                ...
                <div className="fixed inset-0 flex items-center justify-center p-4">
                    <Dialog.Panel className="mx-auto max-w-md w-full rounded-2xl bg-white dark:bg-neutral-800 p-6 shadow-xl">
                        <div className="flex justify-between items-center mb-4">
                            <Dialog.Title className="text-lg font-semibold text-slate-900 dark:text-neutral-100 flex items-center gap-2">
                        ...
                        <div className="mb-6 p-4 bg-slate-50 dark:bg-neutral-900 rounded-lg text-center">
                            <div className="text-sm text-slate-500 dark:text-neutral-400 mb-1">Applying to</div>
                            <div className="text-2xl font-bold text-slate-900 dark:text-neutral-100">
                                {selectedIds.size} <span className="text-sm font-medium text-slate-500 dark:text-neutral-400">transactions</span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                {/* Search Results Dropdown - similar allocation modal, could be refactored into shared component */}
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows={2}
                                    placeholder="Bulk allocation note..."
                                />
                            </div>
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <Button
                                    ...
                                    className="flex-1"
                                    isLoading={isBulkAllocating}
                                    disabled={!selectedMember || isBulkAllocating}
                                >
                                    Allocate All
                                </Button>
                            </div>
